 ...
 * Whether the PubSubAsynIterator is in listening mode (responding to incoming PubSubEngine events and next() calls).
...
export class PubSubAsyncIterator<T> implements AsyncIterator<T> {
  ...
  public async return(): Promise<IteratorResult<Anything>> {
    this.emptyQueue(await this.allSubscribed)
    return { value: undefined, done: true }
  }

  public async throw(err: Anything) {
    this.emptyQueue(await this.allSubscribed)
  ...
  private async pushValue(event: Anything) {
    await this.allSubscribed
  ...
  private pullValue(): Promise<IteratorResult<Anything>> {
